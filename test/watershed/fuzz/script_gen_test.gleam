...
//// respect the binding design invariant that client 0 never authors operations
//// ("Client 0 never authors operations; it doubles as the observer and the
...
import watershed/fuzz/kernel_fuzz.{
  AddClient, ClientOperation, Disconnect, Reconnect, RollbackOperation,
  StashedOperation,
}
...
pub fn command_generator_can_produce_add_client_test() -> Nil {
  let operation_generator = qcheck.bounded_int(from: -5, to: 5)
  let weights =
    Weights(
      client_operation: 0,
      sequence: 0,
  ...
  qcheck.run(
    config(),
    script_gen.command_generator(operation_generator, 2, weights),
    fn(command) { command |> expect.to_equal(AddClient) },
  )
}

/// No generated `ClientOperation`, `Disconnect`, `Reconnect`,
/// `RollbackOperation`, or `StashedOperation`
/// ever names client 0, for any supported client_count (>= 2, the minimum
/// needed for client 0 to have someone to observe). Uses non-default weights so
/// `rollback_operation`/`stashed_operation` (0 by default) are actually
...
pub fn generated_scripts_never_author_from_client_zero_test() -> Nil {
  ...
  let weights =
    Weights(
      ...
      rollback_operation: 10,
      stashed_operation: 10,
    )

  list.each([2, 3, 5], fn(client_count) {
    qcheck.run(
      config(),
      script_gen.script_generator(operation_generator, client_count, weights),
      fn(script) {
