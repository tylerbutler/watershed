...
pub fn apply_stashed_routes_a_fallback_for_a_no_op_generated_command_test() -> Nil {
  let model = text_model.model()
  let assert Some(apply_stashed) = model.capabilities.apply_stashed
  let no_op_generated = text_model.InsertCommand(0, "", None)
  let #(state, routed) =
    apply_stashed(
...
pub fn planted_concurrent_edit_script_converges_deterministically_test() -> Nil {
  let model = text_model.model()
  let script = [
    ClientOp(1, text_model.InsertCommand(0, "hello", None)),
    Synchronize,
    ClientOp(1, text_model.InsertCommand(5, " world", None)),
    ClientOp(2, text_model.InsertCommand(5, " there", None)),
    Synchronize,
    ClientOp(1, text_model.DeleteRangeCommand(0, 5, None)),
    ClientOp(2, text_model.ReplaceRangeCommand(2, 8, "XY", None)),
    Synchronize,
    ClientOp(1, text_model.AppendCommand("!!!", None)),
    ClientOp(2, text_model.InsertCommand(0, ">>", None)),
    Synchronize,
...
pub fn shared_replica_id_is_caught_test() -> Nil {
  let model = text_model.model()
  let script = [
    ClientOp(1, text_model.InsertCommand(0, "a", None)),
    ClientOp(2, text_model.InsertCommand(0, "b", None)),
    Synchronize,
  ]
  let capabilities =
