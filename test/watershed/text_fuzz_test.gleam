...
/// and the empty string for no-operation coverage) round-trips through
/// sequencing, duplicate delivery, rollback, disconnect/reconnect/resubmit,
/// stash replay, and summary reload (`AddClient`) — all exercised generically
/// by `kernel_fuzz.run`/`script_gen`.
...
pub fn command_json_round_trips_with_and_without_delta_test() -> Nil {
  ...
  let assert Ok(#(_, _, Some(text_kernel.Submission(insert_operation, _)))) =
  ...
  let assert Ok(#(_, _, Some(text_kernel.Submission(delete_operation, _)))) =
  ...
  let assert Ok(#(_, _, Some(text_kernel.Submission(replace_operation, _)))) =
  ...
  let assert #(_, _, Some(text_kernel.Submission(append_operation, _))) =
    text_kernel.append(text_kernel.new(replica_id.new("append")), "日д")
  let assert text_kernel.Append(append_value, append_delta) = append_operation

  let commands = [
    text_model.InsertCommand(9, "generated \"value\"", None),
    text_model.InsertCommand(insert_index, insert_value, Some(insert_delta)),
    text_model.DeleteRangeCommand(8, 3, None),
    text_model.DeleteRangeCommand(delete_start, delete_end, Some(delete_delta)),
    text_model.ReplaceRangeCommand(7, 6, "value", None),
    text_model.ReplaceRangeCommand(
      replace_start,
      replace_end,
      replace_value,
      Some(replace_delta),
    ),
    text_model.AppendCommand("appended \"value\"", None),
    text_model.AppendCommand(append_value, Some(append_delta)),
  ]
...
pub fn apply_stashed_preserves_persisted_delta_and_routes_generated_command_test() -> Nil {
  ...
  let assert Ok(#(_, _, Some(text_kernel.Submission(persisted_operation, _)))) =
    text_kernel.insert(
      text_kernel.new(replica_id.new("persisted")),
      0,
      "persisted 👍",
    )
  let assert text_kernel.Insert(index, value, delta) = persisted_operation
  let persisted = text_model.InsertCommand(index, value, Some(delta))
  let #(state, routed) =
    apply_stashed(
  ...
  state.pending
  |> expect.to_equal([text_kernel.PendingOperation(persisted_operation, 0)])
  let assert Ok(_) = text_kernel.ack_local(state, persisted_operation)

  let generated = text_model.InsertCommand(9, "generated \"value\"", None)
  let #(state, routed) =
    apply_stashed(
  ...
  let assert text_model.InsertCommand(0, "generated \"value\"", Some(delta)) =
    routed
  let routed_operation = text_kernel.Insert(0, "generated \"value\"", delta)
  state.pending
  |> expect.to_equal([text_kernel.PendingOperation(routed_operation, 0)])
  let assert Ok(_) = text_kernel.ack_local(state, routed_operation)
  Nil
}

/// A generated command that happens to be a synchronous no-operation (an empty
...
pub fn apply_stashed_routes_a_fallback_for_a_no_operation_generated_command_test() -> Nil {
  let model = text_model.model()
  let assert Some(apply_stashed) = model.capabilities.apply_stashed
  let no_operation_generated = text_model.InsertCommand(0, "", None)
  let #(state, routed) =
    apply_stashed(
      text_kernel.new(replica_id.new("no-op-client")),
      no_operation_generated,
      kernel_fuzz.SubmitMeta(1, 0),
    )
  let assert text_model.AppendCommand(value, Some(delta)) = routed
  state.pending
  |> expect.to_equal([
    text_kernel.PendingOperation(text_kernel.Append(value, delta), 0),
  ])
}
...
pub fn model_summary_load_rebrands_and_can_ack_test() -> Nil {
  ...
  let assert Ok(#(
    source,
    _,
    Some(text_kernel.Submission(confirmed_operation, _)),
  )) =
    text_kernel.insert(
  ...
  let assert Ok(source) = text_kernel.ack_local(source, confirmed_operation)
  ...
  let assert #(
    loaded,
    _,
    Some(text_kernel.Submission(new_operation, message_id)),
  ) = text_kernel.append(loaded, " new")
  message_id |> expect.to_equal(0)
  let assert Ok(loaded) = text_kernel.ack_local(loaded, new_operation)
...
pub fn planted_concurrent_edit_script_converges_deterministically_test() -> Nil {
  let model = text_model.model()
  let script = [
    ClientOperation(1, text_model.InsertCommand(0, "hello", None)),
    Synchronize,
    ClientOperation(1, text_model.InsertCommand(5, " world", None)),
    ClientOperation(2, text_model.InsertCommand(5, " there", None)),
    Synchronize,
    ClientOperation(1, text_model.DeleteRangeCommand(0, 5, None)),
    ClientOperation(2, text_model.ReplaceRangeCommand(2, 8, "XY", None)),
    Synchronize,
    ClientOperation(1, text_model.AppendCommand("!!!", None)),
    ClientOperation(2, text_model.InsertCommand(0, ">>", None)),
    Synchronize,
...
pub fn shared_replica_id_is_caught_test() -> Nil {
  let model = text_model.model()
  let script = [
    ClientOperation(1, text_model.InsertCommand(0, "a", None)),
    ClientOperation(2, text_model.InsertCommand(0, "b", None)),
    Synchronize,
  ]
  let capabilities =
  ...
  let contains_both =
    KernelModel(
      ...
      gen_operation: model.gen_operation,
      check: model.check,
      canonicalize: None,
      ack_preserves_view: model.ack_preserves_view,
      operation_to_json: model.operation_to_json,
      operation_decoder: model.operation_decoder,
