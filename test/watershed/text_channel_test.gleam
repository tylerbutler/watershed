...
/// A CRDT delta that never legitimately matches a real operation's delta: an
/// empty text authored by a replica no honest client would use. Standing in for
/// a tampered/corrupted server echo, mirroring `same_sequence_delta`'s use of
...
pub fn text_summary_round_trips_test() -> Nil {
  let #(state, operation, _) = must_insert(new_a(), 0, "hi 👋")
...
pub fn text_from_snapshot_rebrands_to_the_loading_replica_test() -> Nil {
  let #(state, operation, _) = must_insert(new_a(), 0, "seed")
  let assert Ok(state) = text_kernel.ack_local(state, operation)
  let summary = channel.TextSummary(state.sequenced)
...
pub fn text_apply_remote_emits_changed_event_test() -> Nil {
  ...
  let #(_, remote_operation, _) = must_insert(new_a(), 0, "hi")

  let assert Ok(#(channel.TextState(kernel), events, owed)) =
    channel.apply_remote(
      local,
      channel.TextOperation(remote_operation),
      no_operation_meta(),
    )
...
pub fn text_apply_remote_wrong_operation_type_is_wrong_channel_type_test() -> Nil {
  let state = channel.new(channel.InitText, replica: "a")
  channel.apply_remote(
    state,
    channel.CounterOperation(counter_kernel.Increment(1)),
    no_operation_meta(),
  )
  |> expect.to_equal(
...
pub fn text_ack_local_rejects_mismatched_local_meta_test() -> Nil {
  let assert channel.TextState(kernel) =
    channel.new(channel.InitText, replica: "a")
  let assert Ok(#(kernel, _, Some(text_kernel.Submission(operation, _)))) =
    text_kernel.insert(kernel, 0, "a")
  let detached = channel.TextState(kernel)
...
pub fn other_channel_ack_rejects_text_meta_test() -> Nil {
  ...
  let #(kernel, _, operation, _) = counter_kernel.increment(kernel, 3)
...
pub fn text_same_shape_matches_identical_insert_test() -> Nil {
  let #(_, operation, _) = must_insert(new_a(), 0, "Ada")
...
pub fn text_same_shape_rejects_changed_insert_diagnostic_test() -> Nil {
  let #(_, operation, _) = must_insert(new_a(), 0, "Ada")
  let assert text_kernel.Insert(_, _, delta) = operation
  let altered = text_kernel.Insert(1, "Eve", delta)

  channel.same_shape(
    channel.TextOperation(operation),
    channel.TextOperation(altered),
  )
  |> expect.to_be_false()
}
...
pub fn text_same_shape_rejects_tampered_insert_delta_test() -> Nil {
  let #(_, operation, _) = must_insert(new_a(), 0, "Ada")
  let assert text_kernel.Insert(index, value, _) = operation
  let tampered = text_kernel.Insert(index, value, tampered_delta())

  channel.same_shape(
    channel.TextOperation(operation),
    channel.TextOperation(tampered),
  )
  |> expect.to_be_false()
}
...
pub fn text_same_shape_matches_identical_delete_range_test() -> Nil {
  ...
  let #(_, operation, _) = must_delete_range(state, 1, 3)
...
pub fn text_same_shape_rejects_changed_delete_range_diagnostic_test() -> Nil {
  let #(state, _, _) = must_insert(new_a(), 0, "hello")
  let #(_, operation, _) = must_delete_range(state, 1, 3)
  let assert text_kernel.DeleteRange(_, _, delta) = operation
...
pub fn text_same_shape_rejects_tampered_delete_range_delta_test() -> Nil {
  let #(state, _, _) = must_insert(new_a(), 0, "hello")
  let #(_, operation, _) = must_delete_range(state, 1, 3)
  let assert text_kernel.DeleteRange(start, end, _) = operation
...
pub fn text_same_shape_matches_identical_replace_range_test() -> Nil {
  let #(state, _, _) = must_insert(new_a(), 0, "hello")
  let #(_, operation, _) = must_replace_range(state, 0, 1, "H")
...
pub fn text_same_shape_rejects_changed_replace_range_diagnostic_test() -> Nil {
  let #(state, _, _) = must_insert(new_a(), 0, "hello")
  let #(_, operation, _) = must_replace_range(state, 0, 1, "H")
  let assert text_kernel.ReplaceRange(_, _, _, delta) = operation
...
pub fn text_same_shape_rejects_tampered_replace_range_delta_test() -> Nil {
  let #(state, _, _) = must_insert(new_a(), 0, "hello")
  let #(_, operation, _) = must_replace_range(state, 0, 1, "H")
  let assert text_kernel.ReplaceRange(start, end, value, _) = operation
...
pub fn text_same_shape_matches_identical_append_test() -> Nil {
  let #(_, operation, _) = must_append(new_a(), "tail")
...
pub fn text_same_shape_rejects_changed_append_diagnostic_test() -> Nil {
  let #(_, operation, _) = must_append(new_a(), "tail")
  let assert text_kernel.Append(_, delta) = operation
...
pub fn text_same_shape_rejects_tampered_append_delta_test() -> Nil {
  let #(_, operation, _) = must_append(new_a(), "tail")
  let assert text_kernel.Append(value, _) = operation
...
pub fn text_same_shape_rejects_cross_constructor_comparisons_test() -> Nil {
  let #(state, insert_operation, _) = must_insert(new_a(), 0, "hello")
  let #(state, delete_operation, _) = must_delete_range(state, 0, 1)
  let #(state, replace_operation, _) = must_replace_range(state, 0, 1, "H")
  let #(_, append_operation, _) = must_append(state, "!")

  channel.same_shape(
    channel.TextOperation(insert_operation),
    channel.TextOperation(delete_operation),
  )
  |> expect.to_be_false()
  channel.same_shape(
    channel.TextOperation(delete_operation),
    channel.TextOperation(replace_operation),
  )
  |> expect.to_be_false()
  channel.same_shape(
    channel.TextOperation(replace_operation),
    channel.TextOperation(append_operation),
  )
  |> expect.to_be_false()
  channel.same_shape(
    channel.TextOperation(append_operation),
...
pub fn text_same_snapshot_compares_canonical_encoding_test() -> Nil {
  let #(state, operation, _) = must_insert(new_a(), 0, "same")
  let assert Ok(state) = text_kernel.ack_local(state, operation)
...
pub fn text_kernel_rollback_is_reachable_from_channel_operations_test() -> Nil {
  ...
  channel.same_shape(channel.TextOperation(first), channel.TextOperation(first))
  |> expect.to_be_true()
}

pub fn text_kernel_stash_replay_reproduces_channel_operation_test() -> Nil {
  let #(_, stashed_operation, _) = must_insert(new_a(), 0, "resumed")

  let #(state, events, replayed_operation, message_id) =
    text_kernel.apply_stashed_operation(new_a(), stashed_operation)

  text_kernel.value(state) |> expect.to_equal("resumed")
  events |> expect.to_equal([text_kernel.TextChanged("resumed")])
  channel.same_shape(
    channel.TextOperation(replayed_operation),
    channel.TextOperation(stashed_operation),
  )
  |> expect.to_be_true()
...
pub fn text_kernel_cache_coherence_holds_after_remote_and_local_edits_test() -> Nil {
  let #(state_a, local_operation, _) = must_insert(new_a(), 0, "a")
  let #(_, remote_operation, _) = must_insert(new_a(), 0, "b")
  let #(state_a, _) = text_kernel.apply_remote(state_a, remote_operation)

  text_kernel.check_cache_coherence(state_a) |> expect.to_equal(Ok(Nil))

  let assert Ok(state_a) = text_kernel.ack_local(state_a, local_operation)
  text_kernel.check_cache_coherence(state_a) |> expect.to_equal(Ok(Nil))
}
