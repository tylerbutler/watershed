...
**Interfaces:**
- Consumes:
  - `Result<T, E>`, `Ok<T, E>`, and Gleam `Error<T, E>` from `build/dev/javascript/watershed/gleam.mjs`.
  - `Option<T>`, `Some<T>`, and `None<T>` from `build/dev/javascript/gleam_stdlib/gleam/option.mjs`.
- Produces:
...
- Produces:
  ...
  - `check:types`: `build:gleam` followed by `tsc --noEmit`.
...
"check:types": "pnpm build:gleam && tsc --noEmit",
```

Replace the duplicated Gleam build commands in `predev` and `prebuild`:

```json
"predev": "pnpm check:types && pnpm generate:snippets",
"prebuild": "pnpm check:types && pnpm generate:snippets && node --strip-types --test src/data/drift-gates.test.ts && node --strip-types --test src/data/copy-gates.test.ts",
```

The standalone `check:types` command must build fresh declarations. Do not
allow it to consume an old `build/` directory without invoking Gleam.
