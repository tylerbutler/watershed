...
7. **Tags are one document-wide `OR-Set` of `"<note>\t<tag>"` pairs.** One channel, no per-note handle plumbing: tag a note with `or_set_add(tags, name <> "\t" <> tag)`, untag with `or_set_remove`, filter the sidebar by scanning `or_set_values`. Note names and tags therefore must not contain a tab, enforced at input. This is the textbook OR-Set feature — a tag you re-add after someone concurrently removes it sticks — and the reason it is in scope at all: it adds a kind the demo story does not cover elsewhere. Deleting a note leaves its pairs behind as harmless orphans, same policy as orphaned channels; the renderer only shows pairs whose note exists.
8. **Sidebar order is one `SharedSequence` of note names.** Create appends the name (`sequence_insert` at the end); drag-to-reorder is `sequence_move`; delete removes the entry best-effort. The renderer draws sequence entries that exist in the note map in sequence order, deduplicated first-occurrence-wins, then any map names missing from the sequence appended alphabetically. That one rule absorbs every inconsistency the two channels can drift into — concurrent creates of the same name yield a duplicate entry (renderer dedupes, no repair), and a name in only one channel still renders. No pinning, no sections; `ponytail:` the reconcile rule is the whole feature, and anything fancier waits for a user.
9. **No preview pane in v1.** The user-visible claim is collaborative editing plus a toolbar; rendering is orthogonal to every CRDT behaviour this demo exists to show. If a pure-Gleam commonmark package verifiably compiles on the JavaScript target, preview is a cheap follow-on rung; hand-rolling a markdown renderer or adding a bundled JS parser via FFI is not on the table.
...
**A document-per-item app shape on CRDT-only kinds.** Note list in an `OR-Map`, one text channel per note, open/close/delete while others edit. This is the shape most people actually want `SharedText` for, it retires most of the "offline field notebook" idea from `docs/demo-ideas.md` (`OrMap` + `SharedText` + presence) in a more familiar framing, and — because every kind in the model has ack-free merge — it is the first example whose whole data model is portable to p2p mode unchanged. Tags on an `OR-Set` and sidebar order on a `SharedSequence` widen that to four kinds in one app, each present because its merge rule is the honest model for the feature, not for coverage's sake.
...
pub const tags: ChannelField(Notebook, schema.OrSetChannel) = ...
pub const order: ChannelField(Notebook, schema.SequenceChannel) = ...
```
...
App state is `OR-Map` + `SharedText` + `OR-Set` + `SharedSequence` only (decision 2). The `ChannelField`s above ride the sequenced root `SharedMap`, and that is the model's entire sequencer dependency. The tag set and the order sequence hold note *names*, not handles — the OR-Map stays the single source of what a name resolves to, and the other two channels only annotate it (decisions 7 and 8's render rules absorb the drift that duplication would otherwise turn into corruption).
...
5. **Re-tag vs. untag.** A removes tag `urgent` from a note while B, having seen it, concurrently re-adds it. The tag survives (OR-Set's observed-remove rule) — the one race that makes OR-Set the right kind rather than a register of tag lists.
6. **Create vs. reorder.** A creates a note (appending to the order sequence) while B drags an existing note to the end. Both converge on the same order, and a doubled concurrent create of the same name yields two sequence entries that the renderer collapses to one (decision 8's dedupe, pinned by a render-rule test rather than repaired in the data).
...
- **MN7 — tags.** Decision 7's OR-Set: tag chips on the open note, a tag filter in the sidebar, tab rejected in names and tags at input. Gate: race 5's convergence test passes; filtering by a tag in one tab matches the other tab's view.
- **MN8 — sidebar order.** Decision 8's sequence: append on create, drag-to-reorder via `sequence_move`, best-effort delete, the reconcile-and-dedupe renderer. Gate: race 6's convergence test passes; the render-rule unit test covers duplicate, sequence-only, and map-only names.
- **MN9 — README + smoke.** README states decision 6's three layers verbatim and documents races 1 and 3 as observed. Smoke test per template; headline assertion is race 2.

## Testing strategy

- **Convergence tests** (`test/convergence_test.gleam`, two in-process clients over `sluice_js`, same harness as `release_checklist_lustre`) for all six races. Each rung gate that mentions concurrency is one of these tests, not a manual two-tab check.
