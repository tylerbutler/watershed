...
## Corrections — 2026-08-10 (recorded while building the demo)

The demo shipped as `examples/work_queue_lustre/`. Five things above turned out stale or wrong; the code, not this document, is authoritative for the shapes below.

1. **WQ1 had already landed before this plan was executed.** `subscribe_ordered_collection` exists on both facades and `watershed_lustre` (commits `5cea5d6`, `3d259c9`); the rung shrank to widening the gate tests to the full add → acquire → release → re-acquire → complete lifecycle (`driver_js_test` / `driver_test`, `subscribe_ordered_collection_observes_the_full_lifecycle_test`). The genuine facade gap was **readers**: `ordered_queue` / `ordered_jobs` existed on `runtime_core` but on neither facade, and without them the Queued and In-progress columns cannot render contents. Both were added, on both facades.
2. **The race could not be asserted via `AcquireOutcome` as written** — the outcome never escaped the kernel (the channel ack path dropped it), `ordered_acquire` returns only the caller-minted id, and a losing acquire emits *no event at all*. Rather than infer the loss from queue-drain heuristics, the outcome is now surfaced through the runtime's `Resolution` mechanism (the claims-waiter pattern): `channel.AcquireResolved`, `ordered_acquire_with_outcome` on both facades, `watershed_lustre.ordered_acquire` as an effect, and a new kernel `Aborted` variant for teardown. The demo's "Claim next" pending state resolves from this outcome, and the race test asserts exactly one `AcquiredItem` / one `QueueEmpty` directly.
3. **The floodgate leave-path risk (WQ2 validation above) was already retired** by `pact_map_pending_drains_on_ungraceful_disconnect_test` (`test/watershed/integration_test.gleam`), which proves an ungraceful close yields a sequenced `"leave"` within its polling budget. The throwaway two-client script became the example's permanent smoke test instead. The residual unknown — the hard-TCP-drop timeout for a socket that never closes — is a floodgate default not readable from this repo.
4. **The data-model sketch doesn't compile.** Schema fields are zero-arg `pub fn`s returning `schema.channel_field(...)` with a phantom document type, not `pub const` — see `examples/work_queue_lustre/src/doc_schema.gleam`.
5. Stale line refs: `runtime_core.handle_leave` is at `:870`, not `:606–635`; ordered events are tagged at `:3494`, not `:3224`. Also `ordered_acquire` is FIFO-head with no per-job targeting, so the screen's "Claim" is a single *Claim next* button on the Queued column, not a per-card action.
