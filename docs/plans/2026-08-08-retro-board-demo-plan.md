...
2. **Votes use `OrMap` tally mode, not `PnCounter`.** Two reasons, and the second is decisive:
   - *Design:* one `PnCounter` channel per note means a channel create on every sticky note and N subscriptions to manage. `TallyMode` gives per-key PN-counter leaves in a single channel with a single subscription. This is what the mode is for.
   - *Availability:* there is **no `subscribe_pn_counter`** on either facade (`watershed.gleam` and `watershed_js.gleam` both stop at `pn_counter_update` / `pn_counter_value`), nor in `watershed_lustre`. Votes would be write-only with no change notification. Closing that gap is real work and belongs to a demo where `PnCounter` is the subject — see the backlog doc.
   Net: the arithmetic on display is genuinely conflict-free CRDT arithmetic, and it costs zero prerequisite rungs.
