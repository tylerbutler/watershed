...
Its rung is a bug fix, not a port. `examples/dice_lustre/src/dice_lustre.gleam:182` calls `watershed_js.clear(watershed_js.root(doc))`: composed, that wipes every child handle in the root map — every panel, in one click. Its `snapshot` (`:198–212`) renders `entries(root)` as an inspector table, which composed would show the showcase's own handles rather than dice's state. One change fixes both (take a `SharedMap`, never reach for the root), and the isolation test is obvious: clear dice, assert the other child handles still resolve. It remains the sharpest illustration of why decision 2 exists.
...
Its `audio.Engine` and `#playhead` subtree also make decision 5's retention rule load-bearing in a second way — a dropped panel would leak an `AudioContext` and a `requestAnimationFrame` loop with no handle to stop either.
