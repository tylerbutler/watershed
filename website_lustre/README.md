...
`website/` remains temporarily for historical parity fixtures scheduled for
removal in the final cleanup.
...
The browser fixtures under `test/fixtures/` record the native site's copy,
...
`pnpm run record:parity`. Review the fixture diff before accepting a new baseline.
...
sheet's `gleam.toml` is a whole-file manifest entry, so Astro and Lustre quote
the same file.
