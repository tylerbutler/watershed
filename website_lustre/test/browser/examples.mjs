...
const { record, site, fixture } = parity(import.meta.url, "astro-examples-parity.json");
...
await withBrowserSite(site, async (browser, origin) => {
  ...
  if (record) {
    ...
    console.log("Recorded Astro examples parity baseline.");
    return;
  }
  ...
  console.log("PASS: examples Astro parity, mobile layout, and keyboard focus.");
});
