...
await withBrowserSite(site, async (browser, origin) => {
  for (const route of routes) {
    ...
    const fixture = resolve(root, `test/fixtures/site-foundations-${route.slug}-contract.json`);
    if (record) {
  ...
  console.log(`PASS: ${routes.map(({ slug }) => slug).join(", ")} foundations page contract.`);
});
