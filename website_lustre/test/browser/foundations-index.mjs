...
const { record, site, fixture } = parity(import.meta.url, "astro-foundations-index-parity.json");
...
await withBrowserSite(site, async (browser, origin) => {
  ...
  if (record) {
    await page.evaluate(() => {
      const nodes = {
        "foundations-intro": ".fh-hero",
        "foundations-ledger": ".fh-ledger",
        "foundations-list": ".fh-list",
      };
  ...
  if (record) {
    ...
    console.log("Recorded Astro foundations index parity baseline.");
    return;
  }
  ...
  console.log("PASS: foundations index Astro parity, navigation, focus, and reveal motion.");
});
