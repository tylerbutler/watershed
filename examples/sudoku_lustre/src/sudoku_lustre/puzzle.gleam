...
pub fn by_id(id: String) -> Result(Puzzle, Nil) {
  puzzles()
  |> list.find(fn(puzzle) { puzzle.id == id })
}
...
pub fn given_at(puzzle: Puzzle, row: Int, column: Int) -> Int {
  value_at(puzzle.givens, row, column)
}

pub fn solution_at(puzzle: Puzzle, row: Int, column: Int) -> Int {
  value_at(puzzle.solution, row, column)
}

pub fn is_given(puzzle: Puzzle, row: Int, column: Int) -> Bool {
  given_at(puzzle, row, column) != 0
}

fn value_at(values: List(Int), row: Int, column: Int) -> Int {
  element_at(values, row * 9 + column)
  |> result.unwrap(0)
}

/// The element at `index`. `gleam/list` has no indexed read, so this module
/// walks the list. The result is `Error(Nil)` for an index past the end.
fn element_at(values: List(a), index: Int) -> Result(a, Nil) {
  case values, index {
    [], _ -> Error(Nil)
    [first, ..], 0 -> Ok(first)
