...
/// The two matches whose winners fill the slots of this match. The result is
/// `Error(Nil)` for a quarterfinal, whose slots hold fixed seeds and not other
/// matches.
pub fn feeders(id: MatchId) -> Result(#(MatchId, MatchId), Nil) {
  case id {
    MatchId(Quarterfinal, _) -> Error(Nil)
    MatchId(Semifinal, 1) ->
      Ok(#(MatchId(Quarterfinal, 1), MatchId(Quarterfinal, 2)))
    MatchId(Semifinal, 2) ->
      Ok(#(MatchId(Quarterfinal, 3), MatchId(Quarterfinal, 4)))
    MatchId(Final, _) -> Ok(#(MatchId(Semifinal, 1), MatchId(Semifinal, 2)))
    MatchId(Semifinal, _) -> Error(Nil)
  }
}
...
/// the whole bracket.
pub fn champion(results: Dict(String, MatchResult)) -> Result(String, Nil) {
  case dict.get(results, match_key(final)) {
    Ok(MatchResult(winner:, ..)) -> Ok(winner)
    Error(_) -> Error(Nil)
  }
}
