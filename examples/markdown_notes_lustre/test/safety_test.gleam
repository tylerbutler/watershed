...
fn should_be_missing(view, name: String) {
...
pub fn a_repeated_error_is_recorded_once_test() {
  started()
...
pub fn distinct_errors_are_all_kept_test() {
  started()
  |> simulate.message(markdown_notes_lustre.SignalingFailed("did not answer"))
...
pub fn the_error_variant_name_is_not_shown_test() {
  let view =
    started()
...
pub fn one_failure_reported_by_two_paths_is_recorded_once_test() {
  started()
  |> simulate.message(
...
pub fn deleting_a_note_asks_before_it_happens_test() {
  let view =
    with_notes()
...
fn delete_button(name: String) {
  query.element(matching: query.attribute("aria-label", "delete " <> name))
}

pub fn an_armed_delete_can_be_called_off_test() {
  let view =
    with_notes()
    |> simulate.message(markdown_notes_lustre.RequestDeleteClicked(
      "field notes",
...
pub fn a_note_can_be_moved_by_keyboard_test() {
  let view =
    with_notes()
...
pub fn moving_past_the_end_does_nothing_test() {
  let view =
    with_notes()
...
fn first_note_should_be(view, name: String) {
  let assert Ok(first) =
    query.find_all(
      in: view,
