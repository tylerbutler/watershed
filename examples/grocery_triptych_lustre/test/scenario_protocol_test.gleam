...
pub fn invitation_round_trip_accepts_stripped_outer_type_test() -> Nil {
  let inbound =
    scenario_protocol.decode(None, invitation_payload(), Some("peer-1"))

  inbound
  |> should.equal(
    Ok(scenario_protocol.Inbound(
      from_peer: "peer-1",
...
pub fn status_round_trip_accepts_matching_outer_type_test() -> Nil {
  let inbound =
...
pub fn mismatching_outer_type_is_dropped_test() -> Nil {
  scenario_protocol.decode(
    Some("presence"),
    invitation_payload(),
    Some("peer-1"),
...
pub fn missing_or_foreign_content_kind_is_dropped_test() -> Nil {
  [
    scenario_protocol.decode(
      Some(scenario_protocol.ripple_type),
      invitation_payload_without_kind(),
      Some("peer-1"),
    ),
    scenario_protocol.decode(
      None,
    ...
    scenario_protocol.decode(
      Some(scenario_protocol.ripple_type),
      invitation_payload_with_kind("presence"),
      Some("peer-1"),
    ),
    scenario_protocol.decode(
      None,
  ...
  |> should.equal([Error(Nil), Error(Nil), Error(Nil), Error(Nil)])
}

pub fn sender_client_id_and_phase_fields_are_required_test() -> Nil {
  [
    scenario_protocol.decode(None, invitation_payload(), None),
    scenario_protocol.decode(
      None,
      invitation_payload_with_run_id(""),
      Some("peer-1"),
    ),
    scenario_protocol.decode(
      None,
      go_payload_without_target_peer(),
      Some("peer-1"),
    ),
    scenario_protocol.decode(None, go_payload(""), Some("peer-1")),
    scenario_protocol.decode(
      None,
      status_payload_with_status("bogus"),
      Some("peer-1"),
    ),
  ]
  |> should.equal([Error(Nil), Error(Nil), Error(Nil), Error(Nil), Error(Nil)])
}

pub fn should_acknowledge_requires_ready_idle_foreign_invitation_test() -> Nil {
...
pub fn run_id_filtering_and_first_ack_selection_test() -> Nil {
...
pub fn go_requires_the_selected_target_and_initiator_test() -> Nil {
...
pub fn targeted_status_requires_the_expected_sender_test() -> Nil {
  let status =
    inbound(
      "selected-peer",
