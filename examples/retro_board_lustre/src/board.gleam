...
pub fn render(
  ...
  let render_col = fn(col: Column) {
    let ids =
      sequences
      |> list.find(fn(entry) { entry.0 == col })
      |> result.map(fn(entry) { entry.1 })
      |> result.unwrap([])
    render_column(col, ids, notes, notes_by_id, votes_by_id)
  }
  RenderedBoard(
...
pub fn cards_for(board: RenderedBoard, col: Column) -> List(NoteCard) {
  case col {
