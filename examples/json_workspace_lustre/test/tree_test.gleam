...
pub fn rows_put_folders_before_documents_test() {
...
pub fn rows_sort_each_block_alphabetically_test() {
  let doc_handle = handle.encode_handle("doc-1")
...
pub fn rows_join_paths_under_the_current_folder_test() {
  let doc_handle = handle.encode_handle("doc-1")
...
pub fn a_non_handle_directory_value_is_marked_corrupt_test() {
  tree.rows("/", [], [#("garbage", json.string("not a handle"))])
  |> should.equal([tree.DocRow("garbage", "/garbage", corrupt: True)])
}

pub fn a_handle_directory_value_is_not_corrupt_test() {
...
pub fn a_deleted_path_covers_itself_test() {
  tree.path_covers("/specs", "/specs") |> should.be_true()
}

pub fn a_deleted_path_covers_a_descendant_test() {
  tree.path_covers("/specs", "/specs/api") |> should.be_true()
}

pub fn a_deleted_path_does_not_cover_a_sibling_test() {
  tree.path_covers("/specs", "/spec") |> should.be_false()
}

pub fn a_deleted_path_does_not_cover_an_unrelated_folder_test() {
  tree.path_covers("/specs", "/notes") |> should.be_false()
}

pub fn deleting_the_root_covers_every_path_test() {
  tree.path_covers("/", "/specs/api") |> should.be_true()
}

// ── JSON paths ───────────────────────────────────────────────────────────────

pub fn value_at_reads_object_and_array_paths_test() {
  let value =
    VObject([
      #("a.b", VString("flat")),
  ...
  tree.value_at(value, [Key("a.b")]) |> should.equal(Some(VString("flat")))
  tree.value_at(value, [Key("a"), Key("b"), Index(0)])
  |> should.equal(Some(VString("nested")))
}

pub fn value_at_returns_none_for_a_stale_path_test() {
  tree.value_at(VObject([]), [Key("missing")]) |> should.equal(None)
  tree.value_at(VArray([VString("first")]), [Index(-1)]) |> should.equal(None)
}
